...
switch ($action) {
    ...
    // First get total count of parent tasks only
    $count_sql = "SELECT COUNT(*) as total FROM tasks t 
                  WHERE t.user_id = ? AND (t.parent_id IS NULL OR t.parent_id = 0)";
    ...
    $parent_tasks = $DB->query($sql, $params);
    
    // Get all subtasks for the current parent tasks
    $tasks = $parent_tasks;
    if (!empty($parent_tasks)) {
        $parent_ids = array_column($parent_tasks, 'id');
        $subtasks_sql = "SELECT t.*, tc.category_name 
                       FROM tasks t 
                       LEFT JOIN task_categories tc ON t.category_id = tc.id 
                       WHERE t.parent_id IN (" . implode(',', $parent_ids) . ")
                       AND t.user_id = ?
                       ORDER BY t.task_order, t.added_on DESC";
        $subtasks = $DB->query($subtasks_sql, [$user_id]);
        if (!empty($subtasks)) {
            $tasks = array_merge($parent_tasks, $subtasks);
        }
    }
    ...
    case 'delete':
        try {
            ...
            $DB->query("DELETE FROM tasks WHERE (id = ? OR parent_id = ?) AND user_id = ?", 
                [$_POST['id'], $_POST['id'], $user_id]);
            $response = ['success' => true];
        } catch (Exception $e) {
            $response = ['success' => false, 'message' => $e->getMessage()];
        }
