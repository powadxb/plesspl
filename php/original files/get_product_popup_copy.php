...
$response = $DB->query(" SELECT * FROM all_supplier_stock WHERE id=? LIMIT 1", [$_POST['id']]);
if (!empty($response)) {
  ...
  if (!empty($response)) {
    ...
    $details['qty'] = $stock_details['qty'];
    $details['cost'] = $stock_details['cost'];
    $details['supplier'] = $stock_details['supplier'];
    $details['supplier_sku'] = $stock_details['supplier_sku'];
    $action = 'update';
  }else{
    ...
    $details['name'] =  $name;
    $details['first_import_category'] = $category;
    ...
    $details['supplier_sku'] = $supplier_sku;
    $details['enable'] = 'y';
    $details['export_to_magento'] = 'n';
    $details['pricing_cost'] = null;
    $details['pricing_method'] = 1;
    $details['tax_rate_id'] = 1;
    $details['retail_markup'] = null;
    $details['trade_markup'] = null;
    $details['fixed_retail'] = null;
    $details['fixed_trade'] = null;
    $action = 'add';
    ...
    <div class="col-sm-4" style="border-right: 3px solid #007bff;">
      ...
      <div class="form-group">
        <select id="u-categories" name="categories" class="form-control dropDownMenu requiredField">
          ...
          <option value="<?=$category_txt?>"><?=$category['pless_main_category']?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="form-group">
