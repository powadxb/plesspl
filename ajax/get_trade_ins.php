...
try {
    ...
    // Connect to repairs database for customer search
    $repairsDB = new PDO(
    ...
    if (!empty($search_customer)) {
        // Search in repairs database customers table
        $customer_query = $repairsDB->prepare("
            SELECT id 
            FROM customers 
            WHERE name LIKE ?
        ");
        $customer_query->execute(["%{$search_customer}%"]);
        $customer_ids = $customer_query->fetchAll(PDO::FETCH_COLUMN);
