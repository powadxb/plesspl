...
try {
    ...
    // Also make condition nullable in trade_in_items_details (will be set during testing)
    try {
        $DB->query("ALTER TABLE trade_in_items_details MODIFY COLUMN `condition` enum('excellent','good','fair','poor') NULL");
        $changes[] = "Made trade_in_items_details.condition nullable - will be set during testing";
    } catch (Exception $e) {
        $changes[] = "Skipped trade_in_items_details.condition: " . $e->getMessage();
    }
